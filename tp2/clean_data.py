...
def clean_product(product: Dict) -> Optional[Dict]:
    ...
    cleaned_nutrition = {
        ...
        "energy_kcal": round(nutrition.get("energy_100g", 0) / 4.184)
        if nutrition.get("energy_100g")
        else 0,
...
def calculate_health_score(nutrition: Dict, nutriscore: str) -> int:
    ...
    nutriscore_bonus = {"A": 20, "B": 10, "C": 0, "D": -10, "E": -20}
...
def import_products(db, products_dir: str = "data/products"):
    ...
    for i, file in enumerate(json_files, 1):
        ...
        if cleaned:
            try:
                ...
                print(
                    f"{Fore.WHITE}[{i}/{len(json_files)}]{Style.RESET_ALL} {Fore.GREEN}{category_name:<30}{Style.RESET_ALL} {count:>4} produits"
                )
            except Exception as e:
                print(
                    f"{Fore.RED}[ERROR]{Style.RESET_ALL} {category_name}: {str(e)[:50]}"
                )
    ...
    print(
        f"{Fore.YELLOW}[INFO]{Style.RESET_ALL} {total_skipped} produits ignorés (données manquantes)"
    )
    ...
    print(
        f"{Fore.CYAN}Total produits:{Style.RESET_ALL}        {collection.count_documents({})}"
    )
    ...
    print(
        f"{Fore.CYAN}Nutriscore A:{Style.RESET_ALL}          {collection.count_documents({'nutriscore': 'A'})}"
    )
    print(
        f"{Fore.CYAN}Nutriscore E:{Style.RESET_ALL}          {collection.count_documents({'nutriscore': 'E'})}"
    )
    print(
        f"{Fore.CYAN}Nova group 1 (min):{Style.RESET_ALL}    {collection.count_documents({'nova_group': 1})}"
    )
    print(
        f"{Fore.CYAN}Nova group 4 (max):{Style.RESET_ALL}    {collection.count_documents({'nova_group': 4})}"
    )
