...
class PostgresBenchmark:
    ...
    def analyze_query(self, name: str, query: str, params=None):
        ...
        print(f"{Fore.GREEN}Résultats:{Style.RESET_ALL} {len(results)} lignes en {duration*1000:.2f}ms\n")
...
def run_benchmarks():
    ...
    results.append(benchmark.analyze_query(
    ...
    for r in results:
        print(f"{Fore.CYAN}{r['name']:<50}{Style.RESET_ALL} {r['duration_ms']:>8.2f}ms ({r['rows']} lignes)")
