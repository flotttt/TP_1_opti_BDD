    ...
    with conn.cursor() as cur:
        ...
        products_data = [
            (
                ...
                p["name"],
                brands_map[p["brand"]],
                categories_map[p["main_category"]],
                p["nutriscore"],
                p["nova_group"],
                p["health_score"],
            )
            ...
            for cat in p["categories"]:
                if cat in categories_map:
                    product_categories_data.append(
                        (p["product_id"], categories_map[cat])
                    )
...
def get_database_stats(conn):
    with conn.cursor() as cur:
        ...
        cur.execute(
            "SELECT COUNT(DISTINCT nutriscore) FROM products WHERE nutriscore IS NOT NULL"
        )
...
def main():
    ...
    for i in range(0, len(products), batch_size):
        ...
        print(
            f"{Fore.WHITE}[PROGRESS]{Style.RESET_ALL} {min(i + batch_size, len(products))}/{len(products)} produits importés"
        )
    ...
    print(
        f"{Fore.CYAN}Nutriscore grades:{Style.RESET_ALL}  {stats['nutriscore_grades']}"
    )
