...
class PostgresOptimizer:
    ...
    def create_partitioned_table(self):
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_part_nutriscore ON products_partitioned(nutriscore)"
            )
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_part_health_score ON products_partitioned(health_score)"
            )
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_part_created_at ON products_partitioned(created_at)"
            )
    ...
    def create_materialized_views(self):
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute(
                "CREATE INDEX idx_mv_nutriscore ON mv_products_stats(nutriscore)"
            )

            cur.execute(
                "DROP MATERIALIZED VIEW IF EXISTS mv_top_healthy_products CASCADE"
            )
            cur.execute("""
            ...
            cur.execute(
                "CREATE INDEX idx_mv_top_health_score ON mv_top_healthy_products(health_score)"
            )

            cur.execute(
                "DROP MATERIALIZED VIEW IF EXISTS mv_category_nutrition CASCADE"
            )
            cur.execute("""
            ...
            cur.execute(
                "CREATE INDEX idx_mv_cat_nutrition ON mv_category_nutrition(category_id)"
            )
    ...
    def create_additional_indexes(self):
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_nutrition_sugars_fat ON nutrition_facts(sugars, fat)"
            )
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_nutrition_proteins_desc ON nutrition_facts(proteins DESC)"
            )
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_products_nutriscore_health ON products(nutriscore, health_score DESC)"
            )
            cur.execute(
                "CREATE INDEX IF NOT EXISTS idx_products_nova_nutriscore ON products(nova_group, nutriscore)"
            )
    ...
    def compare_before_after(self):
        ...
        for name, query in queries:
            ...
            print(
                f"{Fore.GREEN}[RESULT]{Style.RESET_ALL} {result['duration_ms']:.2f}ms ({result['rows']} lignes)\n"
            )
        ...
        for r in results:
            print(
                f"{Fore.CYAN}{r['name']:<55}{Style.RESET_ALL} {r['duration_ms']:>8.2f}ms"
            )
        ...
        print(
            f"  Stats catégorie: {Fore.GREEN}{gain_1:+.1f}%{Style.RESET_ALL} ({sans_mv_1:.2f}ms → {avec_mv_1:.2f}ms)"
        )
        print(
            f"  Top produits:    {Fore.GREEN}{gain_2:+.1f}%{Style.RESET_ALL} ({sans_mv_2:.2f}ms → {avec_mv_2:.2f}ms)"
        )

    def analyze_explain_plans(self):
        print(f"\n{Fore.MAGENTA}{'=' * 80}{Style.RESET_ALL}")
        print(
            f"{Fore.MAGENTA}Analyse des plans d'exécution (optimisé){Style.RESET_ALL}"
        )
        print(f"{Fore.MAGENTA}{'=' * 80}{Style.RESET_ALL}\n")
...
def main():
    print(f"\n{Fore.MAGENTA}{'=' * 80}{Style.RESET_ALL}")
    print(
        f"{Fore.MAGENTA}PostgreSQL - Phase 3 : Optimisations avancées{Style.RESET_ALL}"
    )
    print(f"{Fore.MAGENTA}{'=' * 80}{Style.RESET_ALL}\n")
