...
class MongoDBBenchmark:
    ...
    def benchmark_query(self, name: str, query: Dict, projection: Dict = None, sort: List = None, limit: int = None):
    ...
    def explain_query(self, name: str, query: Dict, projection: Dict = None, sort: List = None, limit: int = None):
        ...
        if stage == 'COLLSCAN':
            print(f"  {Fore.RED}⚠ Collection Scan (pas d'index utilisé){Style.RESET_ALL}")
...
def run_phase4_before_optimization():
    ...
    stats = benchmark.explain_query(
        ...
        {
            "nutrition_per_100g.sugars": {"$lt": 5},
            "nutrition_per_100g.fat": {"$lt": 3}
        },
        projection={"name": 1, "brand": 1, "nutrition_per_100g.sugars": 1, "nutrition_per_100g.fat": 1},
        limit=200
    )
    results_before.append({
    ...
    for r in results_before:
        print(f"{Fore.CYAN}{r['name']:<30}{Style.RESET_ALL} {r['time']:>8.2f}ms | Docs: {r['docs_examined']:>6} | Keys: {r['keys_examined']:>6}")
...
def create_mongodb_indexes():
    ...
    indexes = [
        ...
        ("nutrition_sugars_fat", [("nutrition_per_100g.sugars", 1), ("nutrition_per_100g.fat", 1)], False),
...
def run_phase4_after_optimization():
    ...
    stats = benchmark.explain_query(
        ...
        projection={"name": 1, "brand": 1, "nutriscore": 1, "score": {"$meta": "textScore"}},
    ...
    for b, a in zip(before, after):
        ...
        if isinstance(before_time, (int, float)) and isinstance(after_time, (int, float)):
            if before_time > 0:
                ...
                print(f"{Fore.CYAN}{b['name']:<30}{Style.RESET_ALL} {before_time:>7.2f}ms → {after_time:>7.2f}ms {color}({gain:+.1f}%){Style.RESET_ALL}")
            else:
                print(f"{Fore.CYAN}{b['name']:<30}{Style.RESET_ALL} {before_time:>7.2f}ms → {after_time:>7.2f}ms")
